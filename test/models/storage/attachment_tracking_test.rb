...
class Storage::AttachmentTrackingTest < ActiveSupport::TestCase
  ...
  test "board calculate_real_storage_bytes includes comment embeds" do
    ...
    comment_bytes = @board.send(:comment_embed_bytes)

    assert comment_bytes > 0, "comment_embed_bytes should be > 0"
    assert_equal blob.byte_size, comment_bytes
    assert board_bytes >= comment_bytes, "board bytes should include comment bytes"
  end
